...
drbd_dummy_release() {
	...
	dummy-release.sh drbd "$version" drbd-kernel.spec

	sed -re "s/(#define REL_VERSION) .*/\1 \"${version}-1\"/g" drbd/linux/drbd_config.h > drbd/linux/drbd_config.h.tmp
	mv drbd/linux/drbd_config.h{.tmp,}

	for i in 7 8; do
		sed -re "s/(ENV DRBD_VERSION) .*/\1 ${version}-1/g" docker/Dockerfile.centos${i} > docker/Dockerfile.centos${i}.tmp
		mv docker/Dockerfile.centos${i}{.tmp,}
	done

	cat > ChangeLog << EOF
${version}-1 (api:genl2/proto:86-117/transport:14)
--------
