...
int drbd_connected(struct drbd_device *device)
{
	...
	if (!err)
		err = drbd_send_state(first_peer_device(device));
					       ...
					       enum epoch_event ev)
{
	...
	if (schedule_flush) {
		...
		if (fw) {
			...
			fw->epoch = epoch;
			fw->w.device = device;
			drbd_queue_work(&first_peer_device(device)->connection->data.work, &fw->w);
		} else {
...
STATIC int receive_Barrier(struct drbd_connection *connection, struct packet_info *pi)
{
	...
	device = vnr_to_mdev(connection, pi->vnr);
	if (!device)
		return -EIO;
...
STATIC int receive_DataReply(struct drbd_connection *connection, struct packet_info *pi)
{
	...
	err = recv_dless_read(first_peer_device(device), req, sector, pi->size);
	if (!err)
...
STATIC int receive_RSDataReply(struct drbd_connection *connection, struct packet_info *pi)
{
	...
	if (get_ldev(device)) {
		...
		drbd_send_ack_dp(first_peer_device(device), P_NEG_ACK, p, pi->size);
	}
...
STATIC int receive_DataRequest(struct drbd_connection *connection, struct packet_info *pi)
{
	...
	if (!get_ldev_if_state(device, D_UP_TO_DATE)) {
		...
		switch (pi->cmd) {
		case P_DATA_REQUEST:
			drbd_send_ack_rp(first_peer_device(device), P_NEG_DREPLY, p);
			break;
		case P_RS_DATA_REQUEST:
		case P_CSUM_RS_REQUEST:
		case P_OV_REQUEST:
			drbd_send_ack_rp(first_peer_device(device), P_NEG_RS_DREPLY , p);
			break;
		case P_OV_REPLY:
			verb = 0;
			dec_rs_pending(device);
			drbd_send_ack_ex(first_peer_device(device), P_OV_RESULT, sector, size, ID_IN_SYNC);
			break;
		default:
			BUG();
		}
...
STATIC int drbd_asb_recover_0p(struct drbd_device *device) __must_hold(local)
{
	...
	case ASB_DISCARD_ZERO_CHG:
		if (ch_peer == 0 && ch_self == 0) {
			rv = test_bit(DISCARD_CONCURRENT, &first_peer_device(device)->connection->flags)
				? -1 : 1;
			break;
		} else {
...
STATIC int receive_req_state(struct drbd_connection *connection, struct packet_info *pi)
{
	...
	if (test_bit(DISCARD_CONCURRENT, &first_peer_device(device)->connection->flags) &&
	    mutex_is_locked(device->state_mutex)) {
		drbd_send_sr_reply(first_peer_device(device), SS_CONCURRENT_ST_CHG);
		return 0;
	}
	...
	drbd_send_sr_reply(first_peer_device(device), rv);
...
STATIC int receive_UnplugRemote(struct drbd_connection *connection, struct packet_info *pi)
{
	...
	drbd_tcp_quickack(first_peer_device(device)->connection->data.socket);
...
STATIC int got_BarrierAck(struct drbd_connection *connection, struct packet_info *pi)
{
	...
	tl_release(first_peer_device(device)->connection, p->barrier, be32_to_cpu(p->set_size));
...
STATIC int got_OVResult(struct drbd_connection *connection, struct packet_info *pi)
{
	...
	if (device->ov_left == 0) {
		...
		if (w) {
			w->cb = w_ov_finished;
			w->device = device;
			drbd_queue_work_front(&first_peer_device(device)->connection->data.work, w);
		} else {
