...
int w_e_reissue(struct drbd_work *w, int cancel) __releases(local)
{
	...
	/* As long as the -ENOTSUPP on the barrier is reported immediately
	   ...
	   with WO_bdev_flush */
	if (previous_epoch(device->tconn, peer_req->epoch))
	...
	default:
		...
		drbd_may_finish_epoch(device->tconn, peer_req->epoch, EV_PUT + EV_CLEANUP);
		drbd_free_peer_req(device, peer_req);
...
static int e_end_block(struct drbd_work *w, int cancel)
{
	...
	if (peer_req->flags & EE_IS_BARRIER) {
		epoch = previous_epoch(device->tconn, peer_req->epoch);
		if (epoch)
			drbd_may_finish_epoch(device->tconn, epoch, EV_BARRIER_DONE + (cancel ? EV_CLEANUP : 0));
	}
...
static int receive_Data(struct drbd_tconn *tconn, struct packet_info *pi)
{
	...
	if (device->tconn->write_ordering == WO_bio_barrier &&
