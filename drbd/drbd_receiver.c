...
{
	if (atomic_read(&mdev->local_cnt) >= mdev->tconn->net_conf->unplug_watermark)
		drbd_kick_lo(mdev);
}
					       ...
					       enum epoch_event ev)
{
	...
	if (schedule_flush) {
		...
		if (fw) {
			fw->w.cb = w_flush;
			fw->epoch = epoch;
			drbd_queue_work(&mdev->tconn->data.work, &fw->w);
		} else {
...
int w_e_reissue(struct drbd_conf *mdev, struct drbd_work *w, int cancel) __releases(local)
{
	...
	/* As long as the -ENOTSUPP on the barrier is reported immediately
	   ...
	   with WO_bdev_flush */
	if (previous_epoch(mdev, peer_req->epoch))
	...
	err = drbd_submit_ee(mdev, peer_req, WRITE, DRBD_FAULT_DT_WR);
	switch (err) {
	case -ENOMEM:
...
STATIC int e_end_block(struct drbd_conf *mdev, struct drbd_work *w, int cancel)
{
	...
	if (peer_req->flags & EE_IS_BARRIER) {
		epoch = previous_epoch(mdev, peer_req->epoch);
		if (epoch)
			...
			unsigned int data_size)
{
	...
	if (mdev->write_ordering == WO_bio_barrier &&
...
int drbd_asender(struct drbd_thread *thi)
{
	...
	while (get_t_state(thi) == RUNNING) {
		...
		if (likely(rv > 0)) {
			...
			if (mdev->tconn->meta.socket->sk->sk_rcvtimeo ==
			    mdev->tconn->net_conf->ping_timeo*HZ/10) {
