...
extern int drbd_send_state_(struct drbd_device *mdev, const char *func, unsigned int line);
extern int drbd_send_sync_param(struct drbd_device *mdev);
extern void drbd_send_b_ack(struct drbd_device *mdev, u32 barrier_nr,
			    u32 set_size);
...
extern void _drbd_uuid_new_current(struct drbd_device *mdev) __must_hold(local);
...
extern void drbd_go_diskless(struct drbd_device *mdev);
...
extern unsigned long drbd_bm_ALe_set_all(struct drbd_device *mdev,
...
extern void wait_until_done_or_disk_failure(struct drbd_device *mdev, unsigned int *done);
...
void drbd_bump_write_ordering(struct drbd_device *mdev, enum write_ordering_e wo);
...
/* drbd_actlog.c */
extern void drbd_al_begin_io(struct drbd_device *mdev, struct drbd_interval *i);
...
static inline void __drbd_chk_io_error_(struct drbd_device *mdev, int forcedetach, const char *where)
{
 ...
 * @bdev:	Meta data block device.
 */
static inline sector_t drbd_md_ss__(struct drbd_device *mdev,
