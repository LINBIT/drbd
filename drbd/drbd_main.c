 ...
 * @device:	DRBD device.
 */
int drbd_send_current_state_(struct drbd_device *device, const char *func, unsigned int line)
{
	struct drbd_socket *sock;
	struct p_state *p;
 ...
 */
int drbd_send_state_(struct drbd_device *device, union drbd_state state, const char *func, unsigned int line)
{
	struct drbd_socket *sock;
	struct p_state *p;
...
static void drbd_unplug_fn(struct request_queue *q)
{
	...
	if (device->state.pdsk >= D_INCONSISTENT && device->state.conn >= C_CONNECTED) {
		D_ASSERT(device->state.role == R_PRIMARY);
...
enum drbd_ret_code drbd_create_minor(struct drbd_connection *connection, unsigned int minor, int vnr)
{
	...
	q->queue_lock = &connection->req_lock; /* needed since we use */
#ifdef blk_queue_plugged
