 ...
 * @mdev:	DRBD device.
 */
int drbd_send_state_(struct drbd_device *mdev, const char *func, unsigned int line)
{
	struct drbd_socket *sock;
	struct p_state *p;
...
void drbd_send_b_ack(struct drbd_device *mdev, u32 barrier_nr, u32 set_size)
{
	struct drbd_socket *sock;
...
static void drbd_uuid_move_history(struct drbd_device *mdev) __must_hold(local)
{
	int i;
...
void drbd_go_diskless(struct drbd_device *mdev)
{
	D_ASSERT(mdev->state.disk == D_FAILED);
