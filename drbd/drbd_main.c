 ...
 * device->tconn->newest_tle points to the head, device->tconn->oldest_tle points to the tail
		...
		if (n_writes) {
			if (what == RESEND) {
				...
				if (b->w.cb == NULL) {
					...
					inc_ap_pending(b->w.device);
					set_bit(CREATE_BARRIER, &b->w.device->flags);
				}
...
void drbd_send_b_ack(struct drbd_device *device, u32 barrier_nr, u32 set_size)
{
	struct drbd_socket *sock;
	...
	drbd_send_command(device, sock, P_BARRIER_ACK, sizeof(*p), NULL, 0);
}
...
void drbd_mdev_cleanup(struct drbd_device *device)
{
	...
	D_ASSERT(list_empty(&device->tconn->data.work.q));
	D_ASSERT(list_empty(&device->tconn->meta.work.q));
...
enum drbd_ret_code conn_new_minor(struct drbd_tconn *tconn, unsigned int minor, int vnr)
{
	...
	q->queue_lock = &device->tconn->req_lock;
	...
	if (!idr_pre_get(&tconn->volumes, GFP_KERNEL))
		goto out_idr_remove_minor;
	if (idr_get_new_above(&tconn->volumes, device, vnr, &vnr_got))
		goto out_idr_remove_minor;
	if (vnr_got != vnr) {
...
void drbd_go_diskless(struct drbd_device *device)
{
	...
	if (!test_and_set_bit(GO_DISKLESS, &device->flags))
		drbd_queue_work(&device->tconn->data.work, &device->go_diskless);
}
			  ...
			  char *why, enum bm_flag flags)
{
	...
	if (atomic_read(&device->ap_bio_cnt) == 0) {
		if (!test_and_set_bit(BITMAP_IO_QUEUED, &device->flags))
			drbd_queue_work(&device->tconn->data.work, &device->bm_io_work.w);
	}
	spin_unlock_irq(&device->tconn->req_lock);
}
...
STATIC void md_sync_timer_fn(unsigned long data)
{
	...
	drbd_queue_work_front(&device->tconn->data.work, &device->md_sync_work);
}
