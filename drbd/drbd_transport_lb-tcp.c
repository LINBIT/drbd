			      ...
			      enum drbd_stream st, struct dtl_flow **rh_fl)
{
	...
	for_each_path_ref(drbd_path, transport) {
		...
		bool established, have_header = false;
		struct socket *sock;
		...
		if (!sock)
			continue;
		established = sock->sk->sk_state == TCP_ESTABLISHED;
		if (established) {
...
static void dtl_socket_free(struct drbd_transport *transport, struct dtl_flow *flow)
{
	 ...
	 * change to this socket's owner.  On a control socket that owner is
	 * dtl_control_state_change(), which would read our own close as a peer
	 * disconnect, so hand the socket back to its original callback first.
	 */
	write_lock_bh(&s->sk->sk_callback_lock);
	if (s->sk->sk_state_change == dtl_control_state_change)
